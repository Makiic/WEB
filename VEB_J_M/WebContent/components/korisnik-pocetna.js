...
Vue.component("korisnik-pocetna", {
	...
	computed: {
		foundKorpa() {
  return this.korpe.find(korpa => korpa.korisnikId === this.korisnickoIme);
},
...
<div>  	
  ...
  `,
	...
	methods: {
		...
		addToCart(voziloId) {
			// Find the korpa object for the current user
			const korpa = this.korpe.find(korpa => korpa.korisnikId === this.korisnickoIme);

			if (korpa) {
				korpa.vozila.push(voziloId); // Append the selected vehicle ID
				console.log("Added vehicle to cart:", voziloId);
				...
				// Send a PUT request to update the cart
				axios.put('rest/korpe/edit/' + this.korisnickoIme, requestData)
					.then(response => {
						const updatedKorpa = response.data;
						console.log("Updated korpa:", updatedKorpa);
						// Update this.korpa with the server's response if needed
					})
					.catch(error => {
				...
				console.error("Korpa not found for user:", this.korisnickoIme);
				// Handle the case where korpa is not found for the user
			}
		},
